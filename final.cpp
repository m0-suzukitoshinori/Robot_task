...
float T_d(float a4, float b4) {
	float s4 = sr2 * a4 / b4;
	return round(s4 * 10) / 10;
}

int main() {
	ifstream datafile("Planets'deta.txt");

	vector<Star> vec;

	cout << "太陽系の各惑星表面で，入力された速さで地面から45度の方向に質点の物体を投げたときの鉛直方向の最高点とその到達時間，投げた物体が地面の高さと同じのなるときの水平移動距離とその到達時間を表示します．" << endl;
	cout << "任意の投げ速さ[V(m/s)≧0]を入力して，ENTERを押してください．\n" << endl;
	...
	if (v >= 0) {
		...
		cout << "水平移動距離が小さい順に並び替えるはsmallと入力し，ENTERを押してください．\n";
